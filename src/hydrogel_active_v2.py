"""HYDROGEL_PACK active market-maker, v2.

Builds on `src/hydrogel_active_v1.py`. Adds three verified-useful
directional signals consolidated into a single bias score, plus a
critical fix to inventory management that we discovered in the v2/v1
official runs.

Signals (verified on round-4 data):
- Rolling-mean reversion (was strategy 6).
- Spread-regime mean reversion (strategy 4): when spread <= 10 and the
  mid is stretched > 5 ticks from the rolling mean, expect mean
  reversion of ~1 tick at 100-tick horizon.
- Tick-shock reversion (strategy 9): when mid moves >= 5 ticks within
  the last 3 ticks, expect ~0.3 tick mean reversion at 100-1000 ticks.

Behavior changes:
- Single-sided quoting (strategy 2): when bias is strong, the unfavored
  side's inventory budget is dramatically reduced.
- Skew decoupling: when the bias matches the position direction, the
  inventory skew (which normally pulls quotes back toward neutral) is
  suppressed. This unblocks the rolling-mean inventory carry that v1/v2
  could not actually take advantage of.
- Top-of-book (k=0) layer dropped (it added no fills in the official
  runs because bot taker clips never reach that deep).
- Imbalance taker thresholds lowered (combo 1.0 -> 0.5, consecutive
  2 -> 1, cooldown 100 -> 80) to fire roughly 5-10x more often.

Discarded by data verification (do not use):
- Aggressive join at mid+1: zero historical trades at +-1 of mid.
- HYDROGEL/VEV_4000 cross-hedge: correlation ~0 at every horizon.
- Wall-break signal: no qualifying events in historical data.
