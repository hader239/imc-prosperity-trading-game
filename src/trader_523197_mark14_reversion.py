...
class Trader:
    """523197 voucher engine plus a reversion-aware Mark 14/Mark 38 module.

    The original 523197 voucher short engine is kept untouched. For
    HYDROGEL_PACK and VEV_4000, the spread-capture quotes from the
    Mark 14 liquidity probe are kept as the base, but the EMA of mid
    is used to bias clip sizes and capacity. When mid sits below EMA
    we lean into buys (Mark 38 is the persistent seller); when mid
    sits above EMA we lean into sells. This is the moderate variant:
    base clips stay close to the validated probe.
    """
    ...
    def liquidity_reversion_orders(self, product, od, position, ema):
        ...
        # Reversion bias: positive when mid below EMA (favor buys).
